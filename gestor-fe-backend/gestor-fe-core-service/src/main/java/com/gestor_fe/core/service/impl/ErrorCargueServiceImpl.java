...
public class ErrorCargueServiceImpl implements ErrorCargueService {
    ...
    public Iterable<ErrorCargue> saveAll(Iterable<ErrorCargue> errores) {
        if (errores == null) return List.of();
        // ⚡ saveAllAndFlush escribe directamente los registros en la base de datos independientemente de la transacción del Step
        List<ErrorCargue> guardados = repository.saveAllAndFlush(errores);
        return guardados;
    }
    ...
    public byte[] exportErrorCargueToExcel(Long identificadorCargue) throws IOException {
        ...
        if (errores == null || errores.isEmpty()) {
            throw new IllegalArgumentException("No se encontraron registros de error para este cargue.");
        }
