...
@Repository
public interface CausalDevolucionRepsitory extends JpaRepository<CausalDevolucion, Long>{

	Page<CausalDevolucion> findByDeletedAtIsNull(Pageable pageable);

	@Query("select x from CausalDevolucion x where deletedAt is null and upper(x.descripcion) like upper(concat('%', ?1, '%'))")
